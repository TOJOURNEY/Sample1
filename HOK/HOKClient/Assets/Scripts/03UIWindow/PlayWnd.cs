...
public partial class PlayWnd : WindowRoot
{
    ...
    private Vector2 lastStickDir = Vector3.zero;
    ...
    public bool TurnOnFixedSend = true;
    private PEVector3 lastLogicDir = PEVector3.zero;
    private PEVector3 logicDir = PEVector3.zero;

    private void FixedUpdate()
    {
        if(TurnOnFixedSend&&lastLogicDir!=logicDir)
        {

            bool isSend = BattleSys.Instance.SendMoveKey(logicDir);
            if (isSend)
            {
                lastLogicDir = logicDir;
            }
        }
    }
